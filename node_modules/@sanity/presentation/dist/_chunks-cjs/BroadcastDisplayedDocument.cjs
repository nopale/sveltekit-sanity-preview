"use strict";
var reactCompilerRuntime = require("react-compiler-runtime"), react = require("react"), index = require("./index.cjs"), DisplayedDocumentBroadcaster = require("./DisplayedDocumentBroadcaster.cjs");
function BroadcastDisplayedDocument(props) {
  const $ = reactCompilerRuntime.c(7), setDisplayedDocument = DisplayedDocumentBroadcaster.useDisplayedDocumentBroadcaster(), params = index.usePresentationParams(!1);
  let t0;
  $[0] !== props.value || $[1] !== setDisplayedDocument ? (t0 = () => {
    const timeout = setTimeout(() => setDisplayedDocument == null ? void 0 : setDisplayedDocument(props.value), 10);
    return () => clearTimeout(timeout);
  }, $[0] = props.value, $[1] = setDisplayedDocument, $[2] = t0) : t0 = $[2];
  const t1 = params == null ? void 0 : params.perspective;
  let t2;
  return $[3] !== props.value || $[4] !== setDisplayedDocument || $[5] !== t1 ? (t2 = [t1, props.value, setDisplayedDocument], $[3] = props.value, $[4] = setDisplayedDocument, $[5] = t1, $[6] = t2) : t2 = $[6], react.useEffect(t0, t2), null;
}
var BroadcastDisplayedDocument$1 = react.memo(BroadcastDisplayedDocument);
exports.default = BroadcastDisplayedDocument$1;
//# sourceMappingURL=BroadcastDisplayedDocument.cjs.map
