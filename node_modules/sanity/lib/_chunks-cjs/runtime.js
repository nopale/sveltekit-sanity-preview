...
function getSanityPkgExportAliases(sanityPkgPath) {
  ...
  return browserCompatibleSanityPackageSpecifiers.reduce((acc, next) => {
    var _a;
    const dest = (_a = resolve__default.default.exports(pkg, next, {
      browser: !0,
      conditions
    })) == null ? void 0 : _a[0];
...
function renderDocument(options) {
  return new Promise((resolve2, reject) => {
    ...
    const worker = new node_worker_threads.Worker(__filename, {
      execArgv: void 0,
      workerData: __spreadProps$1(__spreadValues$1({}, options), {
        dev: !1,
        shouldWarn: !0
      }),
...
function getDocumentHtml(studioRootPath, props, importMap) {
  ...
  return debug$2("Rendering document component using React"), `<!DOCTYPE html>${addTimestampedImportMapScriptToHtml(server.renderToStaticMarkup(/* @__PURE__ */ jsxRuntime.jsx(Document, __spreadProps$1(__spreadValues$1(__spreadValues$1({}, defaultProps), props), { css }))), importMap)}`;
}
...
function sanityBuildEntries(options) {
  ...
  return {
    ...
    async generateBundle(_options, outputBundle) {
      var _a;
      const bundle = outputBundle, entryFile = Object.values(bundle).find((file) => {
        var _a2;
...
async function getViteConfig(options) {
  ...
  const {
    ...
    importMap,
    reactCompiler
  } = options, monorepo = await loadSanityMonorepo(cwd), basePath = normalizeBasePath(rawBasePath), sanityPkgPath = (_a = await readPkgUp__default.default({
    cwd: __dirname
  })) == null ? void 0 : _a.path;
  if (!sanityPkgPath)
  ...
  const customFaviconsPath = path__default.default.join(cwd, "static"), defaultFaviconsPath = path__default.default.join(path__default.default.dirname(sanityPkgPath), "static", "favicons"), staticPath = `${basePath}static`, {
    ...
    resolve: {
      alias: monorepo != null && monorepo.path ? await getMonorepoAliases(monorepo.path) : getSanityPkgExportAliases(sanityPkgPath),
  ...
  return mode === "production" && (viteConfig.build = __spreadProps(__spreadValues({}, viteConfig.build), {
    assetsDir: "static",
...
function suppressUnusedImport(warning) {
  var _a, _b;
  return warning.code !== "UNUSED_EXTERNAL_IMPORT" ? !1 : !!((_a = warning.names) != null && _a.includes("useDebugValue") && (warning.names = warning.names.filter((n) => n !== "useDebugValue"), warning.names.length === 0) || (_b = warning.ids) != null && _b.every((id) => id.includes("/node_modules/")));
}
